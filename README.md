...
fetch is deliberately split from the parse so failures abort provisioning
instead of silently leaving 80/443 blocked: a failed download stops the script
under `set -e`, and the `jq -e` flag fails the parse when the response
contains no ranges.
